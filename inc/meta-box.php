...
function cover_options($post)
{
  ...
  <label><?php _e('Video ID (leave empty if selfhosted)', 'Minimal-Artistic-Portfolio'); ?></label><br/>
  ...
  <input id="provider_youtube" type="radio" name="video_provider" value="self" <?php echo $video_provider == 'self' ? ' checked' : ''; ?>/>
  <label for="provider_youtube"><?php _e('Self hosted', 'Minimal-Artistic-Portfolio'); ?></label><br/>

  <label><?php _e('Self hosting URLs', 'Minimal-Artistic-Portfolio'); ?></label><br/>
  <label for="576pVideoUrl">576p video URL</label><br/>
  <input id="576pVideoUrl" type="url" name="576pVideoUrl" value="<?php echo isset($firstResUrl) ? $firstResUrl : ''; ?>">
  <br/>
  
  <label for="720pVideoUrl">720p video URL</label><br/>
  <input id="720pVideoUrl" type="url" name="720pVideoUrl" value="<?php echo isset($secondResUrl) ? $secondResUrl : ''; ?>">
  <br/>

  <label for="1080pVideoUrl">1080p video URL</label><br/>
  <input id="1080pVideoUrl" type="url" name="1080pVideoUrl" value="<?php echo isset($thirdResUrl) ? $thirdResUrl : ''; ?>">
  <br/>
...
function save_project($post_id)
{
    ...
    if ($_POST["video_id"] !== get_post_meta($post_id, "VIDEO_ID", true)) {
        $video_id = htmlspecialchars($_POST['video_id']);
        update_post_meta($post_id, "VIDEO_ID", $video_id);
    }
    if ($_POST["576pVideoUrl"] !== get_post_meta($post_id, "576P_VIDEO_URL", true)) {
        update_post_meta($post_id, "576P_VIDEO_URL", htmlspecialchars($_POST["576pVideoUrl"]));
    }
    if ($_POST["720pVideoUrl"] !== get_post_meta($post_id, "720P_VIDEO_URL", true)) {
        update_post_meta($post_id, "720P_VIDEO_URL", htmlspecialchars($_POST["720pVideoUrl"]));
    }
    if ($_POST["1080pVideoUrl"] !== get_post_meta($post_id, "1080P_VIDEO_URL", true)) {
        update_post_meta($post_id, "1080P_VIDEO_URL", htmlspecialchars($_POST["1080pVideoUrl"]));
    }

    if (
    !empty($_POST["video_provider"])
    && in_array($_POST["video_provider"], array( 'vimeo', 'youtube', 'self'))
  ) {
